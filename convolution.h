...
ConvolutionMod1000000007(const std::vector<atcoder::modint1000000007> &a,
    ...
    for (int i = 0; i < n + m - 1; i++) {
        ...
        long long v2 = ((z[i] - ((ModInt3) (x[i] + p1 * v1)).val()) * (ModInt3) m1m2_inv_mod_p3).val();
...
ArbitraryModConvolution(const std::vector<atcoder::modint> &a, const std::vector<atcoder::modint> &b) {
    int n = (int) a.size(), m = (int) b.size();
    ...
    for (int i = 0; i < n + m - 1; i++) {
        ...
        long long v2 = ((z[i] - ((ModInt3) (x[i] + p1 * v1)).val()) * m1m2_inv_mod_p3).val();
